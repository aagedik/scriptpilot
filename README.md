# ScriptPilot Production Deployment Guide

This repository has been hardened for the ScriptPilot production launch. Use the checklist below when preparing new environments.

## Repository & Branching

1. Initialize a private GitHub repository and push the contents of this project.
2. Protect `main` for production. Use feature branches for development work and squash merge into `main`.
3. Never commit secrets, generated assets, or local SQLite files. The provided `.gitignore` already blocks these paths.

## Environment Files

- Copy `.env.example` to `.env.development` for local work and `.env.production` for servers.
- Keep Shopify credentials, PostgreSQL URLs, session secrets, and email API keys outside of source control.
- Deploy the relevant file to each VPS and keep permissions restricted to the deploy user.

## Database (PostgreSQL)

- Prisma is configured to use PostgreSQL via `DATABASE_URL`.
- Provision separate databases for development (`scriptpilot_dev`) and production (`scriptpilot_prod`).
- Run `npm run prisma:migrate:deploy` after updating the schema and before starting the app.

## Build & Runtime Scripts

- `npm run deploy:prepare` — Runs Prisma migrations, generates the client, and builds the Remix app.
- `npm run start:pm2` — Launches the production server through PM2 using `ecosystem.config.cjs` and `.env.production`.
- `npm run pm2:reload` / `npm run pm2:stop` — Manage the running process after deployment.

## VPS Deployment Checklist

1. Clone the private GitHub repo onto the VPS (e.g., `/var/www/scriptpilot`).
2. Copy `.env.production` to the project root and populate production credentials.
3. Install dependencies: `npm ci` (recommended for locked dependencies).
4. Run `npm run deploy:prepare`.
5. Start the app with `npm run start:pm2` or reload with `npm run pm2:reload`.
6. Configure Nginx to proxy to the PM2 process on port 3000 and secure traffic through Cloudflare.

## Security Notes

- Production logging avoids verbose debug output; sensitive data is never written to stdout.
- Webhook HMAC validation is enforced, and admin sessions require strong secrets.
- Remove any unused development tunnels or sample credentials before going live.
...
ScriptPilot uses [Prisma](https://www.prisma.io/) with a PostgreSQL datasource configured via the `DATABASE_URL` environment variable.
The schema is defined in `prisma/schema.prisma`, and migrations are deployed with `npm run prisma:migrate:deploy`.

Any production deployment should target a managed PostgreSQL instance (for example DigitalOcean, Render, or Supabase) with isolated credentials per environment.
